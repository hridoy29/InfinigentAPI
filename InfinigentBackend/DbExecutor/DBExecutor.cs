...
namespace DbExecutor
{
    public class DBExecutor : IDBUtility
    {
        ...
        {
            var fetchrecord = new Dictionary<string, string>();
            try
            {
                OpenConnection();
            ...
            int key_as_column_index, int value_as_column_index)
        {
            ...
            try
            {
                ...
                while (dbDataReader.Read())
                    fetchrecord.Add(dbDataReader.GetValue(key_as_column_index).ToString(),
                        dbDataReader.GetValue(value_as_column_index).ToString());
                if (null != dbDataReader)
                    dbDataReader.Dispose();
            }
            catch (DbException excpt)
        ...
        private string strOutputParamName = string.Empty; // use to retunr the value from stored procedure
        private string strOutputParamValue = string.Empty; // use to retunr the value from stored procedure
        ...
        public int ExecuteScalar32(bool blTransaction, CommandType cmdType, string cmdText, Parameters[] cmdParms,
            bool blDisposeCommand)
        {
            var result = 0;
            try
        ...
        public long ExecuteScalar64(bool blTransaction, CommandType cmdType, string cmdText, Parameters[] cmdParms,
            bool blDisposeCommand)
        {
            long result = 0;
            try
        ...
        public long ExecuteScalar64Ref(bool blTransaction, CommandType cmdType, string cmdText, Parameters[] cmdParms,
            bool blDisposeCommand, ref string text)
        {
            long result = 0;
            var result2 = 0;
            decimal result3 = 0;
            try
            {
                ...
                if (dbDataReader.Read()) result3 = dbDataReader.GetDecimal(0);
                if (dbDataReader.NextResult() && dbDataReader.Read()) text = dbDataReader.GetString(0);
                if (null != dbDataReader)
                    dbDataReader.Dispose();
            }
            catch (DbException except)
        ...
        public string ExecuteScalarString(bool blTransaction, CommandType cmdType, string cmdText,
        ...
        public int ExecuteStoredProcedure(bool blTransaction, CommandType cmdType, string cmdText,
            Parameters[] cmdParms, bool blDisposeCommand)
        {
            var result = 0;
            try
        ...
        public object ExecuteNonQueryForReturnValue(bool blTransaction, CommandType cmdType, string cmdText,
            Parameters[] cmdParms, bool blDisposeCommand)
        {
            object result = null;
            try
            ...
            try
            {
                ...
                while (dbDataReader.Read())
                {
                    ...
                    foreach (var prop in obj.GetType().GetProperties())
                        if (!Equals(dbDataReader[prop.Name], DBNull.Value))
        ...
        public List<T> FetchData<T>(CommandType cmdType, string cmdText, Parameters[] param) where T : new()
        {
            ...
            try
            {
                ...
                var columns = Enumerable.Range(0, dbDataReader.FieldCount).Select(dbDataReader.GetName).ToList();
                ...
                while (dbDataReader.Read())
                {
                    ...
                    foreach (var prop in obj.GetType().GetProperties())
                    {
                        ...
                        if (!string.IsNullOrEmpty(columnName) && !Equals(dbDataReader[prop.Name], DBNull.Value))
                            prop.SetValue(obj, dbDataReader[prop.Name], null);
                    }
        ...
        public List<T> FetchDataRef<T>(CommandType cmdType, string cmdText, Parameters[] param, ref int rows)
            where T : new()
        {
            var list = new List<T>();
            try
            {
                ...
                if (dbDataReader.NextResult() && dbDataReader.Read()) rows = dbDataReader.GetInt32(0);
                if (null != dbDataReader)
                    dbDataReader.Dispose();
            }
            catch (DbException excpt)
        ...
        public DBExecutor()
        {
            try
            {
                ...
                ConnectionStringsSection connectionStringsSection = GetConnectionStringsSection();
                dbProviderFactory =
                    DbProviderFactories.GetFactory(connectionStringsSection.ConnectionStrings["dbCon"].ProviderName);
